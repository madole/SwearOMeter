...
var InputForm = React.createClass({displayName: 'InputForm',
  getSwearsFromServer: function(event) {
    ...
    $.getJSON('http://afternoon-citadel-9782.herokuapp.com/getFucks?username='+ username +'', function(obj) {
      ...
      var twitterUsername = '<a href="http://twitter.com/'+ username + '"> @' + username +'</a>';

      result.html(resultText);
      result.addClass('animated fadeIn');
     // showTweetCycler(obj.tweets);
     _this.refs.spinner.showSpinner(false);
     _this.refs.result.fillInData(twitterUsername, resultText);
...
var Result = React.createClass({displayName: 'Result',
  ...
  render: function() {
    return (
      React.DOM.div(null, 
        React.DOM.div({id: "user", dangerouslySetInnerHTML: this.state.username}), 
        React.DOM.div({id: "result"}, this.state.result)
      )
    )
  }
});
